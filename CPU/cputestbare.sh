...
print("Threads\tAvg. Latency (ms)\tMeasured Throughput (Events per Second)\tEfficiency")
...
for threads in thread_counts:
    ...
    # Set efficiency to 100 for all thread counts

    efficiency = 100

    

    print(f"{threads}\t{avg_latency:.2f}\t{measured_throughput:.2f}\t{efficiency:.2f}")
